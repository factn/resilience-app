...
const MissionsDelivered = ({ currentUser, missions }) => {
  ...
  useFirestoreConnect(() => {
    return [Mission.fsDeliveredUserMissions(org.uid, currentUser.uid)];
  });
  return (
    <Page title="Awaiting Confirmation">
      <Box className={classes.content}>
        <MissionList
          missions={missions}
          isEmpty={isEmpty(missions)}
          isLoaded={isLoaded(missions)}
          isEmptyText="No delivered missions (yet)."
        />
      </Box>
    </Page>
  );
...
const mapStateToProps = (state) => {
  return {
    ...
    missions: state.firestore.ordered.deliveredUserMissions,
  };
};
